...
const statusInfo: Record<string, { label: string; color: string }> = {
  pending:  { label: "Pendiente",  color: "bg-cat-ambar-fondo text-cat-ambar" },
  approved: { label: "Aprobado",   color: "bg-cat-verde-fondo text-cat-verde" },
  rejected: { label: "Rechazado",  color: "bg-cat-rojo-fondo text-cat-rojo" },
};
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted">
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground">Usuario</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground">Plan actual</th>
                <th className="text-left px-5 py-3 font-semibold text-muted-foreground">Plan solicitado</th>
            ...
            <tbody className="divide-y">
              ...
              {filtered.map((s) => {
                const info = statusInfo[s.status] ?? { label: s.status, color: "bg-muted text-muted-foreground" };
                const planId = getPlanId(s.data);
                ...
                return (
                  <tr key={s.id} className="hover:bg-muted transition-colors">
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-foreground">{s.user?.email ?? s.vendor_id}</p>
                      {s.user?.business_name && (
                        <p className="text-xs text-muted-foreground">{s.user.business_name}</p>
                      )}
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="font-mono text-xs bg-muted px-2 py-0.5 rounded text-muted-foreground capitalize">
                        {currentPlan ?? "—"}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="font-mono text-xs bg-cat-azul-fondo px-2 py-0.5 rounded text-cat-azul font-semibold capitalize">
                        {planId}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                      ...
                      {s.status === "pending" ? (
                        <div className="flex gap-2">
                          <button
                            ...
                            disabled={isLoading}
                            className="flex items-center gap-1 px-3 py-1.5 bg-success text-white rounded-lg text-xs font-semibold hover:bg-success/90 disabled:opacity-40 transition-colors"
                          >
                            {isLoading ? (
                          ...
                          <button
                            ...
                            disabled={isLoading}
                            className="flex items-center gap-1 px-3 py-1.5 bg-destructive text-white rounded-lg text-xs font-semibold hover:bg-destructive/90 disabled:opacity-40 transition-colors"
                          >
                            <X className="w-3 h-3" />
                            Rechazar
                          </button>
